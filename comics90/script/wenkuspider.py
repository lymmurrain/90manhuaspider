...
class BaiduWK(object):
    ...
    def get_doc_type_and_title(self):
        ...
        p_list = etree.HTML(content).xpath('//div[@class="bd doc-reader"]/div//text()')


        filename = self.title
        plist = []
        # soup = BeautifulSoup(source_html, "html.parser").decode('gbk')
        # plist.append(soup.title.string)
        # # for div in soup.find_all('div', attrs={"class": "bd doc-reader"}):
        # #     plist.extend(div.get_text().split('\n'))
        # p_list.extend(p_list[1].split('\n'))
        # p_list = [c.replace(' ', '') for c in plist]
        # p_list = [c.replace('\x0c', '') for c in plist]
...
class BDWKDOC(BaiduWK):
    ...
    def get_pure_addr_list(self):
        ...
        # 从源码中批量提取数据url
        all_addr = re.findall('(https.*?0.json.*?)\\\\x22}', source_html)

        url_list = [addr.replace("\\\\\\/", "/") for addr in all_addr]
        patch_sum = len(url_list)//2
        url_list = url_list[0:patch_sum]

        # 获取文档标题
        self.title = etree.HTML(source_html).xpath("//title/text()")[0].strip()

        # 将处理好的url列表保存为全局属性
        self.pure_addr_list = url_list
...
url = 'https://wenku.baidu.com/view/cd7c1f607e21af45b307a846.html'
main(url)